...
class MultiMicroscope:
    ...
    #     mask = np.sum(s, axis=(0,1,2)) > 0 # 1e-10  # remove small singular values
    #     self.u = u[...,mask]
    #     self.s = s[...,mask]
    #     self.vh = vh[...,mask,:]

    def calc_Hxy(self):
        Hxy = np.zeros((self.spang.NX, self.spang.NY, self.jmax,
                      self.data.P, self.data.V), dtype=np.float32)
    ...
    def save_Hxy(self, filename):
        np.savez(filename, Hxy=self.Hxy, Hz=self.Hz)
        
    def load_Hxy(self, filename):
        files = np.load(filename)
        self.Hxy = files['Hxy']
        self.Hz = files['Hz']
    ...
    def fwd(self, f, snr=None):
        ...
        # Tensor multiplication
        outshape = (self.spang.NX, self.spang.NY, self.spang.NZ, self.data.P, self.data.V)
        G = np.zeros(outshape, dtype=np.complex64)
        xx = 0
        for x in tqdm(range(self.spang.NX)):
            for y in range(self.spang.NY):
                for z in range(self.spang.NZ):                
    ...
    #     sreg = np.where(self.s > 1e-7, self.s/(self.s**2 + eta), 0) 
    ...
    #     # Tensor multiplication
    #     Fshift = np.zeros((self.spang.NX, self.spang.NY, self.spang.NZ, self.spang.S), dtype=np.complex64)
    #     for x in tqdm(range(self.spang.NX)):
    #         for y in range(self.spang.NY):
    #             for z in range(self.spang.NZ):
    ...
    #                 HH = np.reshape(np.stack((H0, H1), axis=-1), (self.spang.S, self.data.P*self.data.V)) 
    #                 u, s, vh = np.linalg.svd(HH, full_matrices=False) # Find SVD
    #                 sreg = np.where(s > 1e-7, s/(s**2 + eta), 0) # Regularize
    #                 Fshift[x,y,z,:] = np.einsum('lm,m,mn,n->l', u, sreg, vh, Gshift2[x,y,z,:]) # Apply Pinv
    ...
    def pinv(self, g, eta=0):
        ...
        # 3D FT
        G = np.fft.fftn(g, axes=(0,1,2))
        G2 = np.reshape(G, G.shape[0:3] + (G.shape[3]*G.shape[4],))
        
        # Tensor multiplication
        F = np.zeros((self.spang.NX, self.spang.NY, self.spang.NZ, self.spang.S), dtype=np.complex64)
        for x in tqdm(range(self.spang.NX)):
            for y in range(self.spang.NY):
                for z in range(self.spang.NZ):
                    # Generate H and order it properly
